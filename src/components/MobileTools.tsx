...
export default function MobileTools({
  activeSection, magicMode, drawMode, activeTool,
  onSection, onMagicToggle, onDrawToggle, onEraserClick,
}: Props) {
  ...
  return (
    <div id="mobile-tools">
      <button
        className={`mtbtn${sectionActive && activeSection === 'about' ? ' active' : ''}`}
      ...
      <button
        className={`mtbtn${sectionActive && activeSection === 'projects' ? ' active' : ''}`}
      ...
      <button
        className={`mtbtn${sectionActive && activeSection === 'contact' ? ' active' : ''}`}
      ...
      <button
        className={`mtbtn${sectionActive && activeSection === 'resume' ? ' active' : ''}`}
      ...
      <button
        className={`mtbtn draw${drawMode && activeTool === 'pen' ? ' active' : ''}`}
      ...
      <button
        className={`mtbtn${drawMode && activeTool === 'eraser' ? ' active' : ''}`}
        onClick={onEraserClick}
        title="Eraser"
      >
        <svg viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="1.8">
          <rect x="4" y="9" width="16" height="9" rx="2.5" fill="oklch(65% 0.12 15 / 0.55)" stroke="currentColor"/>
          <rect x="4" y="14" width="16" height="4" rx="1.5" fill="oklch(50% 0.10 15 / 0.45)"/>
          <line x1="4" y1="14" x2="20" y2="14" stroke="currentColor" strokeWidth="0.9"/>
        </svg>
        <span>Eraser</span>
      </button>

      <button
        className={`mtbtn magic${magicMode ? ' active' : ''}`}
